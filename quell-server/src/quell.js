...
class QuellCache {
   ...
   *    - attaches the joined response to the response
   *  object before passing control to the next middleware.
   ...
   *  invoked when QuellCache completes its work
   */
  async query(req, res, next) {
    ...
    if (proto === 'unQuellable') {
      graphql(this.schema, queryString)
        ...
        .catch((error) => next('graphql library error: ', error));
    } else {
      ...
      if (responseFromCache.length === 0) {
        graphql(this.schema, queryString)
          .then(async (queryResponse) => {
            ...
            // append rebuilt response (if it contains data)
            // or fullResponse to Express's response object
        ...
        // if cached response is incomplete, reformulate query, handoff query,
        // join responses, and cache joined responses
        if (Object.keys(queryObject).length > 0) {
          ...
          graphql(this.schema, newQueryString)
            .then(async (queryResponse) => {
              ...
              // rebuild response from cache
              const toReturn = await this.constructResponse(fullResponse, AST, queriedCollection);
              // append rebuilt response (if it contains data) or
              // fullResponse to Express's response object
              res.locals.queryResponse = { data: { [queryName]: toReturn } };
              return next();
            })
            .catch((error) => next('graphql library error: ', error));
      ...
      SelectionSet(node, key, parent, path, ancestors) {
        ...
        function setProperty(path, obj, value) {
          return path.reduce((prev, curr, index) => ((index + 1 === path.length)
            ...
            : prev[curr] = prev[curr] || {}),
            // otherwise, if index exists, keep value or set
            // to empty object if index does not exist
            obj);
        }
  ...
  /**
   * buildFromCache loops over a collection of references retrieved
   *  from cache, calling buildItem on each, and
   * returning an array of those collected items -- that part of
   *  the query that can be satisfied from cache.
   * Simultaneously, buildFromCache and buildItem are updating the
   *  prototype to track which fields still need
   * to be queried.
   * @param {Object} proto - prototype of queried fields
   * @param {Object} map - queryMap if building the main query object,
   *  fieldsMap if building array for nested field
  ...
  /**
   * buildItem iterates through keys -- defined on pass-in prototype
   *  object, which is always a fragment of the
   * prototype, assigning to nodeObject the data at matching keys
   *  in the passed-in item. If a key on the prototype
   * has an object as its value, build array is recursively called.
   * If item does not have a key corresponding to prototype, that
   *  field is toggled to false on prototype object. Data
  ...
  /**
   * createQueryObj traverses the prototype, removing fields
   *  that were retrieved from cache. The resulting object
   * will be passed to createQueryStr to construct a query
   *  requesting only the data not found in cache.
   * @param {Object} proto - the prototype with fields that
   *  still need to be queried toggled to false
   */
  ...
  /**
   * protoReducer is a helper function (recusively) called
   *  from within createQueryObj, allowing introspection of
  ...
  /**
   * createQueryStr converts the query object constructed in
   *  createQueryObj into a properly-formed GraphQL query,
  ...
  /**
   * joinResponses iterates through an array, merging each
   *  item with the same-index item in a second array.
   * joinResponses serves two purposes:
   *   - to merge together objects fetched out of cache
   *  with objects resolved by graphql-js library functions
   *   - to merge together objects in joined response
   *  with object in cache to ensure that no data is lost
  ...
  /**
   * recursiveJoin is a helper function called from within
   *  joinResponses, allowing nested fields to be merged before
   * returning the fully-merged item to joinResponses to be
   *  pushed onto results array. If same keys are present in
   * both cachedItem and uncached Item, uncachedItem -- which
   *  is the more up-to-date data -- will overwrite cachedItem.
    ...
    for (const field in uncachedItem) {
      if (Array.isArray(uncachedItem[field])) {
        if (typeof uncachedItem[field][0] === 'string') {
          const temp = [];
          for (const reference of uncachedItem[field]) {
  ...
  /**
   * generateId generates a unique ID to refer to an item in cache.
   *  Each id concatenates the object type with an
   * id property (user-defined key from this.idMap, item.id or item._id).
   *  If no id property is present, the item is declared uncacheable.
  ...
  /**
   * replaceitemsWithReferences takes an array of objects
   *  and returns an array of references to those objects.
   * @param {String} queryName - name of the query or object type,
   *  to create type-governed references
   ...
   *   - Prevent data loss: Each item of the response
   *  is merged with what is in cache to ensure that no data
   ...
   *  used for identifying response arrays in cache
   */
  async cache(response, collectionName, queryName) {
    ...
    // Write the non-empty array of references to cache
    // (e.g. 'City': ['City-1', 'City-2', 'City-3'...])
  ...
  /**
   * clearCache flushes the Redis cache.
   * To clear the cache from the client, establish an endpoint that
