...
/**
 joinResponses combines two arrays containing results from the cached response and fetched
  (uncached) and outputs a single array response that will ultimately be formatted and
   delivered to the client. the copied Proto parameter sets a reference to combine the
    fields in the same order as the original query.
 */
...
function joinResponses(responseArray, fetchedResponseArray, copiedProto) {
  ...
  for (let i = 0; i < responseArray.length; i += 1) {
    ...
    function fieldRecurse(objStart, objAdd) {
      ...
      for (const field in copiedProto) {
        // if scalar:
        if (typeof copiedProto[field] !== 'object') {
          ...
          // if both objects contain non-scalar fields, join by passing back
          // into joinResponses() or else, add the value from the applicable object that contains it
