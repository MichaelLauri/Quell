...
import { ResultsHelper } from '../helper-functions/HelperFunctions';
...
const Query = (props) => {
  ...
  // show id dropdown (only applies to queries by id)
  const [idDropdown, setIdDropdown] = useState(false);
  // display id dropwodn (only applies to queries by id)
  const [selectedId, setSelectedId] = useState(1);
  // toggle id dropdown menu (only applies to queries by id)
  const [idDropdownMenu, toggleIdDropdownMenu] = useState(false);
  ...
  /*
    ...
    It needs to be formatted essentially for when you run query,
    // so this is a "behind the scenes" function
      ...
      <div className="query-div">
          ...
          <span>
            ...
            >
              <div className="plus-minus-icons dropdown-icon">
                ...
                <img src={DropDownHover} className="hover-button" alt="dropdown" />
              </div>
              {/* Query Dropdown Menu */}
        ...
        <div>
          <QueryFields
            ...
            key={type}
          />
          {/* The above key prop makes it so that when type changes,
          this component completely reloads */}
          {space}
        </div>
