...
/*
  - Connect to Redis server at REDIS_PORT.
    (Redis server must be running and listening for connections
       on your local machine (at the default PORT 6379))
  - Make Redis GET method return a promise in order to use it asynchronously.
    (promisify is a utility function that takes as an argument another function.
...
quellController.quell = (schema) => async (req, res, next) => {
  // client.flushall(); // uncomment if you want to clear the Redis cache

  // If request body does not contain a query,
  // set response to empty object and pass to next middleware
  if (!req.body.query) {
    res.locals.value = {};
    return next();
  }
